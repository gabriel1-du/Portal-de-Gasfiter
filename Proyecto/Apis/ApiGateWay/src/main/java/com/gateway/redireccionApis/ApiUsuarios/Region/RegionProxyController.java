package com.gateway.redireccionApis.ApiUsuarios.Region;
...
@RestController
@RequestMapping("/api/proxy/regionesApi")
@RequiredArgsConstructor
public class RegionProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.regiones.base-url}")
    private String regionesBaseUrl;

    @Value("${services.regiones.base-path}")
    private String regionesBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyRegionesPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxyRegionesSecure(HttpServletRequest request,
                                                 @RequestBody(required = false) String body,
                                                 @RequestHeader HttpHeaders headers) {
        return handleProxy(request, body, headers);
    }
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/regionesApi", "");

        String targetUrl = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(regionesBaseUrl)
                .path(regionesBasePath)
                .path(originalPath)
                .build(true)
                .toUriString();

        HttpMethod method = HttpMethod.valueOf(request.getMethod());
        System.out.println("REGIONES targetUrl: " + targetUrl + "  METHOD: " + method);
