package com.gateway.redireccionApis.ApiUsuarios.TiposUsuarios;
...
@RestController
@RequestMapping("/api/proxy/tiposUsuarioApi")
@RequiredArgsConstructor
public class TiposUsuarioProxyController {

    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.tiposusuario.base-url}")
    private String tiposUsuarioBaseUrl;

    @Value("${services.tiposusuario.base-path}")
    private String tiposUsuarioBasePath;

    @RequestMapping(value = {"", "/**"}, method = {RequestMethod.GET})
    public ResponseEntity<?> proxyTiposUsuarioPublic(HttpServletRequest request,
    ...
    public ResponseEntity<?> proxyTiposUsuarioSecure(HttpServletRequest request,
                                                 @RequestBody(required = false) String body,
                                                 @RequestHeader HttpHeaders headers) {
        return handleProxy(request, body, headers);
    }
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        String originalPath = request.getRequestURI().replace("/api/proxy/tiposUsuarioApi", "");

        String targetUrl = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(tiposUsuarioBaseUrl)
                .path(tiposUsuarioBasePath)
                .path(originalPath)
                .build(true)
                .toUriString();

        HttpMethod method = HttpMethod.valueOf(request.getMethod());
        System.out.println("TIPOS_USUARIO targetUrl: " + targetUrl + "  METHOD: " + method);
