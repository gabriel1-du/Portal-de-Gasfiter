package com.gateway.redireccionApis.ApiUsuarios.PerfilesUsuarios;
...
@RestController
@RequestMapping("/api/proxy/perfilesApi")
@RequiredArgsConstructor
public class PerfilesProxyController {

    
    private final RestTemplate restTemplate;
    private final JwtService jwtService;

    @Value("${services.perfiles.base-url}")
    ...
    private ResponseEntity<?> handleProxy(HttpServletRequest request, String body, HttpHeaders headers) {
        ...
        String targetUrl = org.springframework.web.util.UriComponentsBuilder
                .fromHttpUrl(perfilesBaseUrl)
                .path(perfilesBasePath)
                .path(originalPath)
                .build(true)
                .toUriString();
